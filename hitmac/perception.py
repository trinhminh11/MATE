...
class AttentionLayer(torch.nn.Module):
    def __init__(self, feature_dim, weight_dim = 128):
    ...
    def forward(self, x):
        ...
        # before: z = (batch_size, x camera_num x weight_dim)
        z = torch.bmm(F.softmax(torch.bmm(q, k.permute(0, 2, 1)), dim=2), v)  # [batch_size, sequence_len, weight_dim]
        
        global_feature = z.sum(dim=1) # batch_size x weigh_dim
        return z, global_feature # batch_size x camera_num x weight_dim; batch_size x weight_dim
