...
class EncodeBiRNN(torch.nn.Module):
    ...
    def forward(self, inputs):
        # inputs: (batch_size, seq_len, dim_in)
        x = inputs
...
class EncodeLinear(torch.nn.Module):
    '''
    Args:
        used to encode joint observation (o1, o2,..o_n) before going into the coordinator
        input: n x m + d_input: 
            n: number of agents (sensor)
            m: number of targets 
            d_in: feature dimension per agent-target pair
    '''
...
class A3C_Multi(torch.nn.Module):
    def __init__(self, obs_space, action_spaces, args, device=torch.device('cpu')):
        ...
        # attention to get: global_feature (n_agents x weigh_dim), agents summary view of the world
        # (n_agent x n_target x weigh_dim)
    ...
    def forward(self, inputs, test=False):
        ...
        print(f"Define values:\n{values}\bDefined Action: {actions}")
        return values, actions, entropies, log_probs # action here is goal
